...
class UseStairsAction(Action):
    def perform(self, engine: 'Engine') -> Optional[str]:
        ...
        player_pos = engine.world.component_for_entity(engine.player, engine.components.Position)
        ...
        if current_tile.tile_type == engine.tiles.TileType.STAIRS_DOWN:
            ...
            prev_pos = engine.game_state.get_player_position(engine.game_state.dungeon_level)
            if prev_pos:
