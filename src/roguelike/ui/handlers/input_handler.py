...
class InputHandler:
    ...
    def handle_input(
        self, event: tcod.event.Event, game_state: GameStates = GameStates.PLAYERS_TURN
    ...
    def _handle_player_turn_keys(
        ...
        # Log key details
        logger.debug(
            f"Processing key event: sym={event.sym}, scancode={event.scancode}, mod={event.mod}"
        )
    ...
    def _handle_character_screen_keys(
        self, event: tcod.event.Event
    ) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error(
                f"Error handling character screen key: {str(e)}", exc_info=True
            )
            return None

    def _handle_save_load_keys(
        self, event: tcod.event.Event
    ) -> Optional[Dict[str, Any]]:
