...
def create_lightning_scroll(world, x: int, y: int) -> int:
    ...
    world.add_component(
        ...
        Item(
            name="Lightning Scroll",
            use_function=lambda entity, **kwargs: cast_lightning(
                entity, damage=40, maximum_range=5
            ),
        ),
    )
...
def create_fireball_scroll(world, x: int, y: int) -> int:
    ...
    world.add_component(
        ...
        Item(
            name="Fireball Scroll",
            use_function=lambda entity, **kwargs: cast_fireball(
                entity, damage=25, radius=3
            ),
            targeting=True,
...
def create_shield(
    ...
    world.add_component(
        shield,
        Equipment(equipment_slot=EquipmentSlot.OFF_HAND, defense_bonus=defense_bonus),
    )
...
ITEM_CHANCES: Dict[str, Dict[str, Any]] = {
    ...
    "leather_armor": {
        "chance": 15,
        "create_function": lambda w, x, y: create_armor(
            w, x, y, EquipmentSlot.BODY, "Leather Armor", 1, Colors.BROWN
        ),
    },
    "chain_mail": {
        "chance": 10,
        "create_function": lambda w, x, y: create_armor(
            w, x, y, EquipmentSlot.BODY, "Chain Mail", 2, Colors.LIGHT_GRAY
        ),
    },
    "plate_armor": {
        "chance": 5,
        "create_function": lambda w, x, y: create_armor(
            w, x, y, EquipmentSlot.BODY, "Plate Armor", 3, Colors.SILVER
        ),
    },
    "dagger": {
        "chance": 15,
        "create_function": lambda w, x, y: create_weapon(
            w, x, y, "Dagger", 2, WeaponType.ONE_HANDED, Colors.LIGHT_GRAY
        ),
    },
    "sword": {
        "chance": 10,
        "create_function": lambda w, x, y: create_weapon(
            w, x, y, "Sword", 3, WeaponType.ONE_HANDED, Colors.LIGHT_GRAY
        ),
    },
    "great_sword": {
        "chance": 5,
        "create_function": lambda w, x, y: create_weapon(
            w, x, y, "Great Sword", 4, WeaponType.TWO_HANDED, Colors.LIGHT_GRAY
        ),
    },
    "wooden_shield": {
        "chance": 15,
