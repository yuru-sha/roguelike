...
from roguelike.world.entity.components.base import (
    ...
    Inventory, RenderOrder, EquipmentSlots, EquipmentSlot, WeaponType
)
...
from roguelike.world.entity.prefabs.scroll import create_identify_scroll, create_teleport_scroll
...
def create_player(world: Any, x: int, y: int) -> int:
    ...
    # 初期装備の作成と装備
    # 武器
    dagger = create_weapon(world, 0, 0, "Dagger", 2, WeaponType.ONE_HANDED, Colors.LIGHT_GRAY)
    bow = create_weapon(world, 0, 0, "Short Bow", 2, WeaponType.BOW, Colors.BROWN)
    
    # 防具
    leather_armor = create_armor(world, 0, 0, EquipmentSlot.BODY, "Leather Armor", 1, Colors.BROWN)
    wooden_shield = create_shield(world, 0, 0, "Wooden Shield", 1)
    
    # 消耗品
    healing_potions = [create_healing_potion(world, 0, 0) for _ in range(2)]
    scrolls = [
        create_identify_scroll(world, 0, 0),
        create_teleport_scroll(world, 0, 0)
    ]
