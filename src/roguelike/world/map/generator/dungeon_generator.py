...
from roguelike.world.map.tiles import initialize_tiles, Tile, TileType
    ...
    def generate(self, dungeon_level: int) -> Tuple[List[List[Tile]], Tuple[int, int]]:
        ...
        for _ in range(MAX_ROOMS - 1):
            ...
            for other_room in self.rooms:
                ...
                nearest_room = min(self.rooms, key=lambda r: abs(r.center[0] - new_room.center[0]) + abs(r.center[1] - new_room.center[1]))
        ...
        logger.info(f"Generated dungeon level {dungeon_level} with {len(self.rooms)} rooms")
